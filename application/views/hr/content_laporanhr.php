	...
	<div class="card">
		...
		<div class="card-body">
			...
			<form method="post" action="<?= base_url('hr/overtime/report_hr')?>">
				...
				<table class="table table-striped table-bordered w-100" style="margin-left: 261px;">
					...
					//Looping to print their time of over time save 
							for($d=1;$d < $jmh_hari+1;$d++){
								...
								if($hasil[$x][$d+1] > 0){
									echo '<td><a href="'.base_url('hr/overtime/detail_lembur/'.$hasil[$x][0].'/'.$thn.'-'.$bln.'-'.$d).'" class="badge badge-success">'.$hasil[$x][$d+1].'</a></td>';
								}else if ($hasil[$x][$d+1] < 0){
									echo '<td><a href="'.base_url('hr/overtime/detail_lembur/'.$hasil[$x][0].'/'.$thn.'-'.$bln.'-'.$d).'" class="badge badge-danger">'.$hasil[$x][$d+1].'</a></td>';
								}else if($hasil[$x][$d+1] == 0){
									echo '<td><a href="'.base_url('hr/overtime/detail_lembur/'.$hasil[$x][0].'/'.$thn.'-'.$bln.'-'.$d).'" class="badge badge-light">'.$hasil[$x][$d+1].'</a></td>';
								}else{
									echo '<td><a href="'.base_url('hr/overtime/detail_lembur/'.$hasil[$x][0].'/'.$thn.'-'.$bln.'-'.$d).'">'.$hasil[$x][$d+1].'</a></td>';
								}
							}
