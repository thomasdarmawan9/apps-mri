				...
				<tbody class="text-center">
					...
					foreach($waiting as $row){
						...
						if($row['is_reattendance'] == 1){
                            ...
                            echo "<td nowrap>".$row['paid_date']."</td>";
                        }
						...
						} elseif (!empty($row['edc_detail'])) {
							echo "<td>" . $row['payment_type'] . "<br><label class='badge badge-info'>" . $row['edc_detail'] . "</label></td>";
						}else{
				...
				<div class="modal-body">
					<div class="card">
						...
						<div class="card-body">
								...
								<div class="col-sm-8 row">
									<div class="col-sm-6">
										<div class="custom-control custom-checkbox">
											...
											<label class="custom-control-label" for="std_is_upgrade">Upgrade</label>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="custom-control custom-checkbox">
											<input type="checkbox" class="custom-control-input" id="std_family_discount" name="std_family_discount">
											<label class="custom-control-label" for="std_family_discount">Family Discount</label>
										</div>
									</div>
									<div class="col-sm-6">
										<div class="custom-control custom-checkbox">
									...
									<div class="col-sm-6">
										<div class="custom-control custom-checkbox">
											<input type="checkbox" class="custom-control-input" id="std_free_student" name="std_free_student">
											<label class="custom-control-label" for="std_free_student">Free Student</label>
										</div>
									</div>
								</div>
							...
							<div class="form-group row" id="std_payment_detail">
								<label class="col-form-label col-sm-4"></label>
								<div class="col-sm-3">
									<select class="form-control" name="std_detail_edc" id="std_detail_edc">
										<option value="">-Choose an option-</option>
										<option value="Debit Card">Debit Card</option>
										<option value="Credit Card">Credit Card</option>
									</select>
								</div>
								<div class="col-sm-3" id="std_source_detail">
									<input type="text" class="form-control" name="std_payment_source" id="std_payment_source" placeholder="e.g BCA" autocomplete="off">
								</div>
								<div class="col-sm-2" id="std_detail_cicilan">
									<input type="text" class="form-control" name="std_cicilan" id="std_cicilan" placeholder="e.g 24x" autocomplete="off">
								</div>
							</div>
...
<script>
	$(document).ready(function(){
		...
		std_closing_type();
		std_payment_type();
		std_installment_type();
		payment_type();
		
// 		$('input[type="search"]').on('keyup', function() {
// 			var value = $(this).val();
// 			if (value.length > 0) {
// 				$('#table1').find('tbody').removeClass('d-none')
// 			} else {
// 				$('#table1').find('tbody').addClass('d-none')
// 			}
// 		});
	...
	$('#std_detail_edc').on('change', function() {
		std_installment_type();
	});
	...
	function std_payment_type() {
		...
		if (payment_type == 'EDC') {
			$('#std_payment_detail').show(500);
			$('#std_source_detail').show(500);
		} else {
			$('#std_payment_detail').hide(500);
			$('#std_source_detail').hide(500);
		}
	}

	function std_installment_type() {
		var edc_detail = $('#std_detail_edc').val();
		if (edc_detail == 'Credit Card') {
			$('#std_detail_cicilan').show(500);
		} else {
			$('#std_detail_cicilan').hide(500);
		}
	}
				...
				success:function(result){
					if(result.type == 'new'){
						if(student == 1){
							...
							$('#std_event_name, #std_class, #std_branch').val('').change();
						...
						if(student == 1){
							...
							$("#form_student").trigger('reset');
							$('#std_class, #std_branch').val('').change();
