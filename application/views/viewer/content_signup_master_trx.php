...
<div class="card">
	...
	<div class="card-body">
		<h4>Filter Data</h4>
		<form method="get" action="<?= base_url('viewer/signup/master?')?>">
			<div class="row">
				<div class="col-md-6">
					...
					<div class="form-group row">
						...
						<div class="col-sm-8">
							<select class="form-control js-example-basic-single" name="warrior" id="warrior">
								<option value="all">All</option>
								<?php 
...
<script>
	...
	$('#export').click(function(){
		...
		var url 	= "<?= base_url('viewer/signup/export_data_trx?')?>"+query;
	...
	function init_batch(){
		...
		if(event != 'all'){
			$.ajax({
				url: "<?= base_url('viewer/signup/json_get_batch_list')?>",
				type: "POST",
				dataType: "json",
				data:{'event': event},
	...
	function detail_trx(id){
		...
		$.ajax({
			url: "<?= base_url('viewer/signup/json_master_get_detail_transaction')?>",
			type: "POST",
			dataType: "json",
			data:{'id': id},
			success:function(result){		
