...
<div class="card">
	<div class="card-header">
		<h3 class="card-title">Master Data Participant</h3>
	</div>
	<div class="card-body">
		<h4>Filter</h4>
		<form method="get" action="<?= base_url('viewer/signup/master_participant?')?>" id="form_filter">
			<div class="row">
				<div class="col-md-6">
					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Email</label>
						<div class="col-sm-8">
							<input type="email" name="email" id="email" class="form-control" placeholder="" value="" autocomplete="off">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-sm-8">
							<input type="text" name="phone" id="phone" class="form-control" placeholder="" value="" autocomplete="off">
						</div>
					</div>

					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Age Range</label>
						<div class="col-sm-8">
							<div class="row">
								<div class="col-md-4">
									<input type="number" name="age1" id="age1" min="0" max="100" value="" placeholder="min" class="form-control" autocomplete="off">
								</div>
								<div class="col-md-1">
									-
								</div>
								<div class="col-md-4">
									<input type="number" name="age2" id="age2" min="0" max="100" value="" placeholder="max" class="form-control" autocomplete="off">
								</div>
							</div>
						</div>
					</div>
				...
				<div class="col-md-6">
					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Vege / Alergy</label>
						<div class="col-sm-8">
							<select class="form-control" name="kondisi" id="kondisi">
								<option value="all">None</option>
								<option value="is_vegetarian">Vegetarian</option>
								<option value="is_allergy">Alergy</option>
							</select>
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col-sm-8">
							<input type="text" name="date_start" id="date_start" class="form-control" value="" autocomplete="off">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Date End</label>
						<div class="col-sm-8" id="div_date_end">
							<input type="text" name="date_end" id="date_end" class="form-control" value="" autocomplete="off">
						</div>
					</div>
				</div>
			</div>
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					...
					<button type="button" class="btn btn-blue-grey" onclick="reset()">CLEAR</button>
			...
			<table class="table table-bordered w-100" id="table1">
				<thead class="text-center">
					<tr style="background-color: #0d4e6c ;font-size:0.9rem;color:#fff;">
						...
						<th style="vertical-align: middle">Others</th>
						<th style="vertical-align: middle">Date Created</th>
						<th style="vertical-align: middle">Trx</th>
				...
				<tbody class="text-center">
					...
					foreach($list as $row){
						...
						echo "<td><label class='badge badge-light'>".ucwords($row['participant_name'])."</label></td>";
						echo "<td><label class='badge badge-light'>".$row['phone']."</label></td>";
						echo "<td><label class='badge badge-light'>".$row['email']."</label></td>";
						if($row['age'] > 0){
							echo "<td class='text-center'>".$row['age']."</td>";
						}else{
							echo "<td class='text-center'>-</td>";
						}
						echo "<td><label class='badge badge-light'>".$row['birthdate']."</label></td>";
						if($row['gender'] == "L"){
							echo "<td><label class='badge badge-primary'>Male</label></td>";
						}else{
						...
						if(!$row['is_vegetarian'] && !$row['is_allergy']){
							...
							if($row['is_allergy']){
								echo "<label class='badge secondary-color'>Alergy</label>";
							}
						}
						echo "</td>";
						echo "<td class='text-center'><label class='badge badge-light' style='white-space:normal;'>".($row['date_created'])."</label></td>";
						echo "<td class='text-center'><label class='badge elegant-color-dark'>".$row['jmlh_trx']."</label></td>";
						echo "<td>
...
<div class="modal fade" tabindex="-1" id="modal_edit" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<form action="<?= base_url('viewer/signup/update_participant')?>" method="post" id="form_edit" style="padding-left:15px;padding-right: 15px;">
...
<script>
	...
	$('#export').click(function(){
		...
		var url 	= "<?= base_url('viewer/signup/export_data_participant?')?>"+query;
	...
	function detail(id){
		if(id != ''){
			$.ajax({
				url: "<?= base_url('viewer/signup/json_participant_get_transaction')?>",
				type: "POST",
				dataType: "json",
				data:{'id': id},
	...
	function edit(id){
		...
		if(id != ''){
			$.ajax({
				url: "<?= base_url('viewer/signup/json_get_data_participant_by_id')?>",
				type: "POST",
				dataType: "json",
				data:{'id': id},
				success:function(result){
