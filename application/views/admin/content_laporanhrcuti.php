<div class="card">
	<div class="card-header">
		<h3 class="card-title">Report Cuti Izin</h3>
	</div>
	<div class="card-body">
		<form method="post" action="<?= base_url('admin/overtime/report_izin_hr')?>">
			...
			<div class="form-group row">
				...
				<div class="col-sm-4">
					<select class="form-control" name="thn" required="">
						...
						<option value="<?php echo date('Y'); ?>" <?= (!empty($thn) && date('Y') == $thn)? 'selected' : ''?> ><?php echo date('Y'); ?></option>
						<option value="<?php echo date('Y') - 1; ?>" <?= (!empty($thn) && date('Y') - 1 == $thn)? 'selected' : ''?> ><?php echo date('Y') - 1; ?></option>
						<option value="<?php echo date('Y') - 2; ?>" <?= (!empty($thn) && date('Y') - 2 == $thn)? 'selected' : ''?> ><?php echo date('Y') - 2; ?></option>
					</select>
				</div>
			</div>
			<div class="form-group row">
			...
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr style="background: #f8f9fa;">
						<!-- <td></td> -->
						<td style="font-weight:bold;color:#212529;font-weight:bold;">Total Cuti Tahun Sebelumnya</td>
						<td></td>
						<td></td>
						<td style="color:#212529;font-weight:bold; text-align: center;"><label class="badge badge-info"><?php echo $cutisblm; ?></label></td>
						<td></td>
						<td></td>
					</tr>
					...
					<tr style="background: #0d4e6c;">
						...
						<td></td>
						<td></td>
						<td style="font-weight:bold;color:white;font-weight:bold;text-align: center;"><?php echo $balance; ?></td>
						<td></td>
						<td></td>
					</tr>
	...
	function hapus(id){
		if(id != ''){
			swal({
				...
				if (result) {         
					$.ajax({
						url: "<?= base_url('admin/overtime/delete_cuti_izin')?>",
						type: "POST",
						dataType: "json",
						data:{'id': id},
