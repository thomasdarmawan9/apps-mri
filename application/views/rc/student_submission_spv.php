			...
			<table class="table table-bordered w-100" id="table1">
				...
				<tbody class="text-center">
					...
					foreach($list as $row){
						...
						echo "<td class='text-center'><a onclick='confirm(".$row['change_id'].")' class='btn teal' title='Confirm data?' data-toggle='tooltip' data-placement='top'><i class='fa fa-check'></i></a>
								<a onclick='reject(".$row['change_id'].")' class='btn btn-danger' title='Reject data?' data-toggle='tooltip' data-placement='top'><i class='fa fa-times'></i></a>";
...
<script>
	...
	function confirm(id){
		if(id != ''){
			swal({
				title: 'Konfirmasi',
				text: "Data student akan diupdate beserta absensinya, lanjutkan?",
				type: 'warning',
				...
				if (result) {				  
					$.ajax({
						url: "<?= base_url('rc/student/approve_submission')?>",
						type: "POST",
						dataType: "json",
						data:{'id': id},
	...
	function reject(id){
		if(id != ''){
			swal({
				title: 'Konfirmasi',
				text: "Tolak pengajuan pindah kelas, lanjutkan?",
				type: 'warning',
				showCancelButton: true,
				...
				if (result) {				  
					$.ajax({
						url: "<?= base_url('rc/student/reject_submission')?>",
						type: "POST",
						dataType: "json",
						data:{'id': id},
						success:function(result){					
