...
class Model_batch extends CI_Model {
	...
	public function get_data_peserta($batch_id){
		...
		$this->db->select('transaction_id, event_name, participant_name, TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) AS age, gender, participant.phone, participant.email, paid_value, paid_date, closing_type, user.username');
		$this->db->join('transaction', 'participant.participant_id = transaction.participant_id');
		$this->db->join('user', 'user.id = transaction.id_user_closing');
	...
	public function get_data_export_batch($batch_id){
		$this->db->select('transaction.event_name, batch_label, participant_name, TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) AS age, gender, participant.phone, participant.email');
		$this->db->join('transaction', 'participant.participant_id = transaction.participant_id');
		$this->db->join('user', 'user.id = transaction.id_user_closing');
