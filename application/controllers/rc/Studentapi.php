...
class Studentapi extends CI_Controller {
	...
	public function manage_as_spv(){
        ...
        if(!empty($post['input'])){
            if($post['periode'] != 'all'){
                $periode			= $this->model_periode->get_data($post['periode']);
            }else{
                $periode['periode_start_date'] = '';
                $periode['periode_end_date'] = '';
            }
            $data['list'] 		= $this->model_student->get_filtered_student($post['input'], $periode['periode_start_date'], $periode['periode_end_date'], 'regular');
        }else{
            $data['list'] 		= array();
        }
	...
	public function manage_as_spv_adult(){
        ...
        if(!empty($post['input'])){
            ...
            $data['list'] 		= $this->model_student->get_filtered_student($post['input'], $periode['periode_start_date'], $periode['periode_end_date'], 'adult');
        }else{
            $data['list'] 		= array();
        }
        echo json_encode($data);
	...
	public function submission_spv(){
		# as supervisor
			$data['list'] = $this->model_student->get_data_submission_waiting();
			echo json_encode($data);
	}

	public function submission_trainer(){
			$post = json_decode(file_get_contents('php://input'), true);
			# as trainer
			$data['list'] = $this->model_student->get_data_submission($post['id']);
			echo json_encode($data);
	...
	public function submission_add(){
		...
		if(!empty($post['get'])){
			if($post['periode'] != ''){
				$periode 			= $this->model_periode->get_data($post['periode']);
				// dd($periode);
				$data['list'] 		= $this->model_student->get_student_list($periode['periode_start_date'], $periode['periode_end_date'], $post['branch'], $post['school'], $post['class'], 'regular');
			}else{
				$data['list'] 		= $this->model_student->get_student_list('','', $post['branch'], $post['school'], $post['class'], 'regular');
			}
		}else{
			$data['list'] 		= array();
		}
	...
	public function submission_adult_add(){
		...
		if(!empty($post('get'))){
			if($post('periode') != ''){
				$periode 			= $this->model_periode->get_data($post('periode'));
				// dd($periode);
				$data['list'] 		= $this->model_student->get_student_list($periode['periode_start_date'], $periode['periode_end_date'], $post('branch'), $post('school'), $post('class'), 'adult');
			}else{
				$data['list'] 		= $this->model_student->get_student_list('','', $post('branch'), $post('school'), $post('class'), 'adult');
			}
		}else{
			$data['list'] 		= array();
		}
	...
	public function submit_edit_form(){
		...
		if($result1){
			...
			echo json_encode("edit data error");
		}
	}
	...
	public function delete_student(){
		...
		if($response){
			...
			echo json_encode("data gagal didelete");
		}
	}
